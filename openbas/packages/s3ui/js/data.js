...
var loadedData = 0;
var loadedStreams = {}; // maps a stream's uuid to the total number of points that have been cached for that stream
...
function initData (streams, offsets, startTime, endTime, loadingElem, callback) {
...
function getResolutionStep(stream, resolution) {
...
/* Ensures that STREAM has data cached from STARTTIME to ENDTIME at a specified
   ...
   behind user input). */
var savedCalls = {}; // An object that maps a stream's UUID to a pending call for that stream
var pendingCalls = {}; // An object that maps a stream's UUID to a boolean that indicates if a call is pending
function ensureData(stream, resolution, startTime, endTime, callback) {
...
function boundMemory(streams, offsets, startTime, endTime, target) {
